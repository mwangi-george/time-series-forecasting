...
date: today
description: "A practical walkthrough in R."
...
raw_sales_df <- read.csv(
  ...
  mutate(date = lubridate::mdy(date))  # Parse date column into Date type
...
To better understand recurring patterns in the sales data, we use seasonal diagnostics to examine how sales behave across **months, quarters, and years**.

```{r message=FALSE, fig.height=7, fig.width=12}
# ------------------------------------------------------------------------------
# Seasonal diagnostics
# - Examine sales distributions by month, quarter, and year
# - Helps identify recurring seasonal patterns and long-term shifts
# ------------------------------------------------------------------------------
...
This chart shows Sales for Product Y show clear **annual growth over time**, with stronger performance toward the **second half of the year**.
...
With our time-based features in place, the next step is to define the models we want to evaluate. In this post, we focus on the **XGBoost** machine learning algorithm. Rather than relying on a single configuration, we explore multiple XGBoost models by systematically varying key hyperparameters, such as the **learning rate, tree depth, and regularization strength** to identify the most effective setup.
...
This gives us a total of 320 recipe–model combinations to train
...
From the table above, the top-performing XGBoost hyperparameter configurations achieve a **MAE of \~43k and MAPE of \~18% on the test set**, with several models delivering virtually identical accuracy, indicating a **stable performance plateau** where small hyperparameter changes no longer materially improve forecast quality.
...
future_forecast_tbl |>
  plot_modeltime_forecast(
    .legend_show = FALSE,
    .title = "Final Sales Forecast for Product Y (24-Month Horizon)",
    .y_lab = "Sales in USD"
  )
...
In this post, we walked through an end-to-end machine learning workflow for sales forecasting in R, starting from data exploration and anomaly detection, through feature engineering, model specification, parallel training, evaluation, and finally producing a forward-looking forecast using XGBoost.

This approach offers clear benefits: it leverages rich time-based features, uses a robust validation strategy, and produces forecasts that are both data-driven and reproducible. At the same time, it’s important to recognize the limitations, machine learning models like XGBoost can be sensitive to hyperparameter choices, require careful feature engineering, and may not always generalize best across all datasets or business contexts.
...
[![](img/author.jpeg){width="100"}](https://www.linkedin.com/in/georgemwangikenya/)

*You can connect with George on:*

-   [LinkedIn](https://www.linkedin.com/in/georgemwangikenya)

-   [Website](https://mwangi-george.netlify.app)
